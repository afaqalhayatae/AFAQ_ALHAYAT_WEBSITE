# Service Areas

## Document Information

- **Owner:** Business Owner and Operations
- **Status:** Approved at emirate level; Priority Community Registry (below) approved for publication as of 2026-08-05
- **Version:** 1.1
- **Effective Date:** 2026-07-23 (community-level addendum: 2026-08-05)
- **Authority:** Canonical geographic coverage registry

## Purpose

This document is the only permitted source for geographic service coverage.
Website pages, CRM availability, booking, campaigns, structured data, and AI
answers must reference approved location IDs from this registry.

## Owner Decision

On 2026-07-23, the business owner confirmed that AFAQ Alhayat provides every
service listed in `04_SERVICE_KNOWLEDGE/SERVICE_CATALOG.md` across all seven
emirates of the United Arab Emirates.

This approval applies at **emirate level only**. It does not create or approve:

- Physical branches.
- Street addresses.
- Google Business Profile locations.
- City, district, or community coverage records.
- Response-time commitments.
- Same-day or emergency availability.
- Service availability outside the current canonical catalog.

## Approved Registry

| Area ID | Name EN | Name AR | Type | Parent | Coverage | Services | Effective |
|---|---|---|---|---|---|---|---|
| `LOC-AE-AZ` | Abu Dhabi | أبوظبي | Emirate | `LOC-AE` | Active | All catalog services | 2026-07-23 |
| `LOC-AE-DU` | Dubai | دبي | Emirate | `LOC-AE` | Active | All catalog services | 2026-07-23 |
| `LOC-AE-SH` | Sharjah | الشارقة | Emirate | `LOC-AE` | Active | All catalog services | 2026-07-23 |
| `LOC-AE-AJ` | Ajman | عجمان | Emirate | `LOC-AE` | Active | All catalog services | 2026-07-23 |
| `LOC-AE-UQ` | Umm Al Quwain | أم القيوين | Emirate | `LOC-AE` | Active | All catalog services | 2026-07-23 |
| `LOC-AE-RK` | Ras Al Khaimah | رأس الخيمة | Emirate | `LOC-AE` | Active | All catalog services | 2026-07-23 |
| `LOC-AE-FU` | Fujairah | الفجيرة | Emirate | `LOC-AE` | Active | All catalog services | 2026-07-23 |

`All catalog services` means the stable service IDs in
`04_SERVICE_KNOWLEDGE/SERVICE_CATALOG.md` at the time of use. A newly proposed
service does not inherit coverage until it is added to the approved catalog.

## Community-Level Publication Addendum (2026-08-05)

On 2026-08-05 the business owner directly confirmed, in the course of website
development work, that the named communities in the Priority Community
Registry below may be published as service-area content on the website
(location pages listing them, not standalone pages per community). This
resolves the "lower-level areas pending" status this document previously
carried for those specific, already-named rows only.

This does **not** approve:

- Any community, district, or neighborhood not already listed by name in the
  Priority Community Registry below.
- Standalone landing pages per community (still a distinct SEO-quality-gate
  decision, not made here).
- Branch addresses, response-time commitments, or anything else the Owner
  Decision above already excludes.

## Publication Rules

- UAE-wide and emirate-level coverage may be stated using this registry.
- City, district, community, or neighborhood claims are permitted only for
  rows explicitly listed in the Priority Community Registry (see the
  2026-08-05 addendum above) — any other sub-emirate claim remains blocked.
- Emirate landing pages require unique useful content and the normal SEO quality gate.
- Coverage does not prove availability for a particular date or time.
- Booking must confirm operational availability before making a commitment.
- Google Business Profile configuration must follow service-area-business and
  ownership rules; coverage does not justify creating virtual branches.
- AI answers must distinguish general coverage from confirmed booking availability.

## Priority Community Registry

On 2026-07-23 the owner authorized the project to select premium-property and
high-value communities as marketing priorities and confirmed that all catalog
services may be offered in them.

These are **service areas and marketing priorities, not branch addresses**.
Booking still confirms operational date/time availability.

### Dubai

| Area ID | Name EN | Name AR | Type | Parent | Priority | Services |
|---|---|---|---|---|---|---|
| `LOC-AE-DU-PALM-JUMEIRAH` | Palm Jumeirah | نخلة جميرا | Community | `LOC-AE-DU` | Tier 1 | All catalog services |
| `LOC-AE-DU-EMIRATES-HILLS` | Emirates Hills | تلال الإمارات | Community | `LOC-AE-DU` | Tier 1 | All catalog services |
| `LOC-AE-DU-DUBAI-HILLS` | Dubai Hills Estate | دبي هيلز استيت | Community | `LOC-AE-DU` | Tier 1 | All catalog services |
| `LOC-AE-DU-JUMEIRAH-GOLF-ESTATES` | Jumeirah Golf Estates | عقارات جميرا للجولف | Community | `LOC-AE-DU` | Tier 1 | All catalog services |
| `LOC-AE-DU-AL-BARARI` | Al Barari | البراري | Community | `LOC-AE-DU` | Tier 1 | All catalog services |
| `LOC-AE-DU-JUMEIRAH-BAY` | Jumeirah Bay Island | جزيرة جميرا باي | Community | `LOC-AE-DU` | Tier 1 | All catalog services |
| `LOC-AE-DU-DISTRICT-ONE` | District One | دستركت ون | Community | `LOC-AE-DU` | Tier 1 | All catalog services |
| `LOC-AE-DU-JUMEIRAH-ISLANDS` | Jumeirah Islands | جزر جميرا | Community | `LOC-AE-DU` | Tier 1 | All catalog services |
| `LOC-AE-DU-TILAL-AL-GHAF` | Tilal Al Ghaf | تلال الغاف | Community | `LOC-AE-DU` | Tier 1 | All catalog services |
| `LOC-AE-DU-ARABIAN-RANCHES` | Arabian Ranches | المرابع العربية | Community | `LOC-AE-DU` | Tier 1 | All catalog services |
| `LOC-AE-DU-DOWNTOWN` | Downtown Dubai | وسط مدينة دبي | District | `LOC-AE-DU` | Tier 1 | All catalog services |
| `LOC-AE-DU-DUBAI-MARINA` | Dubai Marina | دبي مارينا | District | `LOC-AE-DU` | Tier 1 | All catalog services |

### Abu Dhabi

| Area ID | Name EN | Name AR | Type | Parent | Priority | Services |
|---|---|---|---|---|---|---|
| `LOC-AE-AZ-SAADIYAT` | Saadiyat Island | جزيرة السعديات | Community | `LOC-AE-AZ` | Tier 1 | All catalog services |
| `LOC-AE-AZ-YAS` | Yas Island | جزيرة ياس | Community | `LOC-AE-AZ` | Tier 1 | All catalog services |
| `LOC-AE-AZ-AL-RAHA-BEACH` | Al Raha Beach | شاطئ الراحة | Community | `LOC-AE-AZ` | Tier 1 | All catalog services |
| `LOC-AE-AZ-AL-BATEEN` | Al Bateen | البطين | District | `LOC-AE-AZ` | Tier 1 | All catalog services |
| `LOC-AE-AZ-JUBAIL-ISLAND` | Jubail Island | جزيرة الجبيل | Community | `LOC-AE-AZ` | Tier 1 | All catalog services |
| `LOC-AE-AZ-AL-MAQTAA` | Al Maqtaa | المقطع | District | `LOC-AE-AZ` | Tier 1 | All catalog services |
| `LOC-AE-AZ-AL-REEM` | Al Reem Island | جزيرة الريم | Community | `LOC-AE-AZ` | Tier 2 | All catalog services |
| `LOC-AE-AZ-KHALIFA-CITY` | Khalifa City | مدينة خليفة | City | `LOC-AE-AZ` | Tier 2 | All catalog services |

### Sharjah

| Area ID | Name EN | Name AR | Type | Parent | Priority | Services |
|---|---|---|---|---|---|---|
| `LOC-AE-SH-AL-ZAHIA` | Al Zahia | الزاهية | Community | `LOC-AE-SH` | Tier 2 | All catalog services |
| `LOC-AE-SH-ALJADA` | Aljada | الجادة | Community | `LOC-AE-SH` | Tier 2 | All catalog services |
| `LOC-AE-SH-TILAL-CITY` | Tilal City | مدينة تلال | Community | `LOC-AE-SH` | Tier 2 | All catalog services |
| `LOC-AE-SH-AL-TAI` | Al Tai | الطي | District | `LOC-AE-SH` | Tier 2 | All catalog services |
| `LOC-AE-SH-MARYAM-ISLAND` | Maryam Island | جزيرة مريم | Community | `LOC-AE-SH` | Tier 2 | All catalog services |

### Ajman and Ras Al Khaimah

| Area ID | Name EN | Name AR | Type | Parent | Priority | Services |
|---|---|---|---|---|---|---|
| `LOC-AE-AJ-AL-ZORAH` | Al Zorah | الزوراء | Community | `LOC-AE-AJ` | Tier 2 | All catalog services |
| `LOC-AE-AJ-CORNICHE` | Ajman Corniche | كورنيش عجمان | District | `LOC-AE-AJ` | Tier 3 | All catalog services |
| `LOC-AE-RK-AL-MARJAN` | Al Marjan Island | جزيرة المرجان | Community | `LOC-AE-RK` | Tier 2 | All catalog services |
| `LOC-AE-RK-MINA-AL-ARAB` | Mina Al Arab | ميناء العرب | Community | `LOC-AE-RK` | Tier 2 | All catalog services |
| `LOC-AE-RK-AL-HAMRA-VILLAGE` | Al Hamra Village | قرية الحمراء | Community | `LOC-AE-RK` | Tier 2 | All catalog services |

### Fujairah and Umm Al Quwain

| Area ID | Name EN | Name AR | Type | Parent | Priority | Services |
|---|---|---|---|---|---|---|
| `LOC-AE-FU-AL-AQAH` | Al Aqah | العقة | District | `LOC-AE-FU` | Tier 3 Test | All catalog services |
| `LOC-AE-FU-AL-FASEEL` | Al Faseel | الفصيل | District | `LOC-AE-FU` | Tier 3 Test | All catalog services |
| `LOC-AE-UQ-UAQ-MARINA` | Umm Al Quwain Marina | مرسى أم القيوين | Community | `LOC-AE-UQ` | Tier 3 Test | All catalog services |
| `LOC-AE-UQ-AL-KHOR` | Al Khor | الخور | District | `LOC-AE-UQ` | Tier 3 Test | All catalog services |

Tier 3 Test areas require a short commercial validation before dedicated paid
campaigns or large location-content investment. General service availability
remains active.

## Future Lower-Level Rows

When added, every row must include:

| Field | Requirement |
|---|---|
| `area_id` | Stable identifier; never reused |
| `name_en` | Approved English name |
| `name_ar` | Approved Arabic name |
| `area_type` | City, district, community, branch, or service zone |
| `parent_area_id` | One approved emirate ID |
| `coverage_status` | Active, limited, planned, suspended, or inactive |
| `service_ids` | Explicit approved services |
| `constraints` | Scheduling, access, or scope restrictions |
| `effective_from` | Approval effective date |
| `reviewed_on` | Most recent owner review |
| `approved_by` | Accountable owner or role |
| `evidence` | Operational evidence supporting coverage |

## Legacy Drafts

The following migration HOLD files remain preserved as historical evidence:

- `00_START/01_BUSINESS/02_BRAND/03_MARKET_RESEARCH/04_SERVICES/18_SERVICE_LOCATIONS.md`
- `00_START/01_BUSINESS/02_BRAND/03_MARKET_RESEARCH/04_SERVICES/19_SERVICE_AREAS_BY_EMIRATE.md`
- `00_START/TEMP/01_BRAND/02_MARKET/SERVICE_AREAS.md`

They are superseded for emirate-level authority by this approved registry.
Their unapproved city and district lists must not be published or silently
merged.

## Review Checklist

- [x] Actual operating emirates confirmed by owner.
- [x] All current catalog services mapped to each emirate.
- [x] Approved English and Arabic emirate names recorded.
- [x] Approve the initial premium and high-value community priorities.
- [ ] Expand and review cities, districts, and communities using operational and commercial evidence.
- [ ] Confirm service-area-business versus physical branch structure.
- [ ] Record Google Business Profile and Place IDs when verified.
- [ ] Validate booking constraints and scheduling by area.
- [ ] Review coverage at least quarterly.

## Related Documents

- `02_BRAND/CONTACT_INFORMATION.md`
- `02_BRAND/LOCAL_SEO_PROFILE.md`
- `04_SERVICE_KNOWLEDGE/SERVICE_CATALOG.md`
- `04_SERVICE_KNOWLEDGE/SERVICE_MATRIX.md`
- `09_AI_KNOWLEDGE/ANSWER_POLICY.md`
- `99_STANDARDS/SEO_STANDARD.md`
